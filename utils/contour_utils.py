...
# for evaluation/postprocessing
def get_consecutive_samples_of_patch_starting_points_and_image_with_bbox_with_stride(mask_image, stride=1,
                                                                                     rgb_image=None):
    bounding_boxes = get_bbox_from_mask_image(mask_image)
    list_starting_points = []
    for x, y, w, h in bounding_boxes:
...
def get_saturation_thresholded_mask_from_tumor_wsi(tumor_wsi, wsi_mask):
    ...
    wsi_mask_grayscale_inv_resized = cv2.resize(wsi_mask_grayscale_inv,
                                                (tumor_wsi_array.shape[1], tumor_wsi_array.shape[0]))
    print("\n wsi_mask_grayscale_inv_resized : ",
          wsi_mask_grayscale_inv_resized.shape, " tumor_wsi : ",
          tumor_wsi_array.shape, " Sizes match ? = ",
          (wsi_mask_grayscale_inv_resized.shape[0] == tumor_wsi_array.shape[0] and wsi_mask_grayscale_inv_resized.shape[
              1] == tumor_wsi_array.shape[1]))
    tumor_wsi_excluding_tumor_area = cv2.bitwise_and(tumor_wsi_array, tumor_wsi_array,
                                                     mask=wsi_mask_grayscale_inv_resized)
    # Image.fromarray(tumor_wsi_excluding_tumor_area).show()

    saturation_thresholded_mask_rgb = get_saturation_thresholded_mask_from_non_tumor_wsi(
        non_tumor_wsi=tumor_wsi_excluding_tumor_area)
...
def get_and_save_patch_samples_from_mask_and_wsi_image(mask_image,  # rgb image of mask
                                                       ...
                                                       patch_resolution_level=None,
                                                       # resolution level for patch extraction
                                                       is_tumor_image=True):
    ...
    for x, y in patch_starting_points:
        ...
        if is_tumor_image:
            ...
            patch_read_from_wsi_at_zero_level = wsi_image_original.read_region((x * scaling_factor, y * scaling_factor),
                                                                               0,
                                                                               (wsi_props.PATCH_SAMPLE_BOX_SIZE,
                                                                                wsi_props.PATCH_SAMPLE_BOX_SIZE))
    ...
    print(" samples_accepted : {0}, \t\t samples_rejected : {1}, \t\t bbox_samples_used : {2}".format(samples_accepted,
                                                                                                      samples_rejected,
                                                                                                      count))
    if is_tumor_image:
...
def get_and_save_normal_patch_samples_from_both_images(mask_image_resolution_level, wsi_path, wsi_mask_path=None,
    ...
    non_tumor_wsi = wsi_image_original.read_region((0, 0), mask_image_resolution_level,
